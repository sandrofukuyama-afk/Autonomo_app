...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _attemptManualExchange() async {
    ...
    if (code != null && AuthService.instance.currentUser == null) {
      setState(() => _exchangeError = null);
      try {
        await AuthService.instance.exchangeCodeForSession(code);
      } catch (e) {
        if (mounted) {
          setState(() => _exchangeError = e.toString().replaceFirst('Exception: ', ''));
          _showMessage(_exchangeError!);
        }
      }
    }
  }
                        ...
                        if (_exchangeError != null)
                          Padding(
                            ...
                            child: Column(
                              children: [
                                Text(
                                  _exchangeError!.toLowerCase().contains('code verifier') 
                                    ? 'Erro: O link expirou ou foi aberto em outra aba/navegador.' 
                                    : 'Erro: $_exchangeError',
                                  style: const TextStyle(color: Colors.red, fontSize: 10, fontWeight: FontWeight.bold),
                                  textAlign: TextAlign.center,
                                ),
                                const SizedBox(height: 4),
                                if (_exchangeError!.contains('Code verifier'))
                                  const Text(
